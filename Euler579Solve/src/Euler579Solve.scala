  ...
  def gen(n:Int) = {
    ...
    for(_case <- 0.to(1))
      for(a <- -n to n if a*a <= 4*n; b <- -n to n if a*a+b*b <= 4*n; c <- -n to n if a*a+b*b+c*c <= 4*n) {
        ...
        val L0 = - √(5*n - a*a - b*b - c*c)
        val R0 = √(5*n - a*a - b*b - c*c)
        val L1 = max(L0, -n)
        val R1 = max(R0, n)
        ...
        if((_case==0 && _t2<0) || (_case==1 && _t1<0)) 0 else {
          ...
          val t = √( if(_case==0) t2 else t1 ) + 1
          val L3 = max(L2, if(_case==0) -t else a-t)
          val R3 = max(R2, if(_case==0) t else a+t)
          val Lj = (if(_case==0) a else 0) - √( if(_case==0) t1 else t2 ) + 1
          val Rj = (if(_case==0) a else 0) + √( if(_case==0) t1 else t2 ) - 1
